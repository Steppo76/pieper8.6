/* Changelog
 * Version 0.8.6.0: Zeitzählung ändern
 * 
 * Version 0.8.5.1: LED wieder aktiviern.
 *                  timeArray wird bei Buttondruck zurückgesetzt.
 * Version 0.8.5.0:
 * 
 */
...
void loop()
{
  ...
  if (pieperState == HIGH)
  {
    ...
    if (sekStrom < 60) // Stromsparmodus
      ...
      aktZeit = myRTC.get() - timeArray[i-1];
//      Serial.println(i);
//      Serial.println(myRTC.get());
//      Serial.println(timeArray[i-1]);
//      Serial.println(aktZeit);
      ...
      screen.print(hour(aktZeit));
      screen.print(":");

      screenDigit(minute(aktZeit));
      screen.print(":");

      screenDigit(second(aktZeit));
